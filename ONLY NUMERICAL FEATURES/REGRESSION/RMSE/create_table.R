...
label <- paste0("TABLES/table_results_RMSE")
...
caption <- paste0("Average test RMSE. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_RMSE_mahalanobis")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
for (task_id in task_ids)
{
  ...
  for (directory in list_directories)
  {
    ...
    filename <- file.path(directory, paste0(task_id, "_random_RMSE_results.csv"))
...
avg_rel_diff <- avg_rel_diff[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_rel_diff) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rel_diff)
...
avg_norm_acc <- avg_norm_acc[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_norm_acc) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_norm_acc)
...
avg_rank <- avg_rank[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_rank) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rank)
...
caption <- paste0("Average test RMSE. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_RMSE_random_splits")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
