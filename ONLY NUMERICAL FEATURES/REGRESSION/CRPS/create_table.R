...
caption <- paste0("Average test CRPS. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_CRPS_mahalanobis")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average test CRPS. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_CRPS_spatial_depth")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average test CRPS. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_CRPS_clustering")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
for (task_id in task_ids)
{
  ...
  for (directory in list_directories)
  {
    if (directory == "RESULTS/RANDOM")
    {
      i=1
    }
    if (directory == "RESULTS/RANDOM_CAT")
    {
      i=2
    }
    filename <- file.path(directory, paste0(task_id, "_random_crps_results.csv"))
    
    # Check if the file exists
    if (file.exists(filename))
    {
      # Load the results dataset
      results_dataset <- head(read.csv(filename),-i)
...
for (directory in list_directories) {
  ...
  for (filename in list.files(directory)) {
    ...
    if (endsWith(filename, ".csv")) {
      ...
      # Read the CSV file into a data frame
      table <- head(read.csv(filepath),-i)
...
k=1
for (method in methods) {
  avg_rel_diff[[method]] <- mean_rel_diff[k]
  k=k+1
}
avg_rel_diff <- avg_rel_diff[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans", "drf", "distributional_boosted_trees")]
colnames(avg_rel_diff) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rel_diff)
...
k=1
for (method in methods) {
  avg_norm_acc[[method]] <- mean_norm_acc[k]
  k=k+1
}
avg_norm_acc <- avg_norm_acc[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans", "drf", "distributional_boosted_trees")]
colnames(avg_norm_acc) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_norm_acc)
...
k=1
for (method in methods) {
  avg_rank[[method]] <- mean_rank[k]
  k=k+1
}
avg_rank <- avg_rank[,c("task_id", "constant", "linear_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans", "drf", "distributional_boosted_trees")]
colnames(avg_rank) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rank)
...
caption <- paste0("Average test CRPS. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_CRPS_random_splits")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
