...
for (task_id in task_ids)
{
  ...
  for (directory in list_directories)
  {
    ...
    {
      filename <- file.path(directory, paste0(task_id, "_spatial_depth_accuracy_results.csv"))
    }
    ...
    if (file.exists(filename))
    {
      ...
      result_Accuracy <- cbind(result_Accuracy, Accuracy)
    }
  }
  ...
  {
    result_Accuracy_mean <- result_Accuracy$Accuracy
  }
...
for (directory in list_directories) {
  ...
  for (filename in list.files(directory)) {
    ...
    if (endsWith(filename, ".csv")) {
      ...
      tmp[1,]<-(highest_Accuracy - Accuracy) / highest_Accuracy
      df <- rbind(df, tmp)
    }
  }
}
...
for (directory in list_directories) {
  ...
  for (filename in list.files(directory)) {
    ...
    if (endsWith(filename, ".csv")) {
      ...
      tmp[1,] <- pmin(pmax((Accuracy - mid_Accuracy) / (highest_Accuracy - mid_Accuracy), 0), 1)
      df <- rbind(df, tmp)
    }
  }
}
...
for (directory in list_directories) {
  ...
  for (filename in list.files(directory)) {
    ...
    if (endsWith(filename, ".csv")) {
      ...
      tmp[1,]<-rank(-1*Accuracy, na.last="keep")
      df <- rbind(df, tmp)
    }
  }
}
...
# Find the lowest and highest values in each row
...
for (i in 1:nrow(output)) {
  if ((i==nrow(output) - 2)|(i==nrow(output)))
  {
  output[i, -1] <- ifelse(output[i, -1] == lowest_values[i], paste0("\\textbf{", output[i, -1], "}"), output[i, -1])
  }
  else
  {
    output[i, -1] <- ifelse(output[i, -1] == highest_values[i], paste0("\\textbf{", output[i, -1], "}"), output[i, -1])
  }
}


caption <- paste0("Average test Accuracy. 
...
label <- paste0("TABLES/table_results_Accuracy")
...
caption <- paste0("Average test Accuracy. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_Accuracy_mahalanobis")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average test Accuracy. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_Accuracy_spatial_depth")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average test Accuracy. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_Accuracy_clustering")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average test Accuracy. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
                   #Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_Accuracy_umap")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
############# ONLY RANDOM SPLITS ####################
############################################
list_directories <- c("RESULTS/RANDOM", "RESULTS/RANDOM_CAT")
...
for (task_id in task_ids)
{
  ...
  for (directory in list_directories)
  {
    filename <- file.path(directory, paste0(task_id, "_random_accuracy_results.csv"))
...
for (directory in list_directories) {
  ...
  for (filename in list.files(directory)) {
    ...
    if (endsWith(filename, ".csv")) {
      ...
      # Read the CSV file into a data frame
      table <- head(read.csv(filepath),-2)
...
avg_rel_diff <- avg_rel_diff[,c("task_id", "constant", "logistic_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_rel_diff) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rel_diff)
...
avg_norm_acc <- avg_norm_acc[,c("task_id", "constant", "logistic_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_norm_acc) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_norm_acc)
...
avg_rank <- avg_rank[,c("task_id", "constant", "logistic_regression", "rf", "boosted_trees", "engression", "MLP", "ResNet", "FTTrans")]
colnames(avg_rank) <- c("task_id", models_new_name)
results <- bind_rows(results, avg_rank)
...
caption <- paste0("Average test Accuracy. 
                  ...
                  'Avg. rank' denotes the average rank of a method.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_Accuracy_random_splits")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
