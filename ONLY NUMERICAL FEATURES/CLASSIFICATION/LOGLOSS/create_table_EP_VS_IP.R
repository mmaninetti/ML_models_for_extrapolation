...
result_logloss_all <- data.frame(method = methods, stringsAsFactors = FALSE)
for (task_id in task_ids)
{
  ...
  for (directory in list_directories)
  {
    ...
    {
      filename <- file.path(directory, paste0(task_id, "_spatial_depth_logloss_results.csv"))
    }
    ...
    if (file.exists(filename))
    {
      ...
      random_filename <- file.path(random_directory, paste0(task_id, "_random_logloss_results.csv"))
      random_dataset <- head(read.csv(random_filename),-i)
      
      # Extract the Method and logloss columns
      random_method <- random_dataset$Method
      random_logloss <- random_dataset$Log.Loss
      
      random_logloss <- ifelse(random_logloss >= 0, random_logloss, NA)
      
      diff <- 100*(logloss - random_logloss) / random_logloss

      # Append the Method and logloss to the result_row
      result_logloss <- cbind(result_logloss, diff)
      result_logloss_all <- cbind(result_logloss_all, diff)
    }
  }
  ...
  {
    result_logloss_mean <- result_logloss$diff
  }
...
rowdiffs <- rowMeans(result_logloss_all[,-1], na.rm=TRUE)
...
rowdiffs <- apply(result_logloss_all[,-1], 1, median, na.rm=T)
...
caption <- paste0("Average relative difference of test logloss using EP VS using IP, in \\%. 
...
label <- paste0("TABLES/table_results_logloss_only_num_features_EP_VS_IP")
...
caption <- paste0("Average relative difference of test logloss using EP VS using IP, in \\%. 
                  ...
                  'Median diff.' denotes the median of each column.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_clustering_logloss_only_num_features_EP_VS_IP")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average relative difference of test logloss using EP VS using IP, in \\%. 
                  ...
                  'Median diff.' denotes the median of each column.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_umap_logloss_only_num_features_EP_VS_IP")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average relative difference of test logloss using EP VS using IP, in \\%. 
                  ...
                  'Median diff.' denotes the median of each column.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_spatial_depth_logloss_only_num_features_EP_VS_IP")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
...
caption <- paste0("Average relative difference of test logloss using EP VS using IP, in \\%. 
                  ...
                  'Median diff.' denotes the median of each column.")
#Bold results are non-inferior to the best result in a paired t-test at a 5\\% level.
label <- paste0("TABLES/table_results_mahalanobis_logloss_only_num_features_EP_VS_IP")
filename <- paste0(label, ".tex", sep="")
tab <- xtable(output, caption = caption, label = label)
